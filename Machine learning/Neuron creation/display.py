...
def draw(line, interval=500):
    ...
    animation = FuncAnimation(figure, step, frames=len(
        gathered_points), interval=interval, repeat=False, init_func=lambda *args, **kwargs: None)
...
def display(*points, line=None, interval=500):
    try:
        displayer = draw(line, interval)
        next(displayer)
        for p in points:
            displayer.send(p)
        next(displayer)
...
#     (100, 100)
# ]
# display(points, line=line)
# labelled = label_points(line, points)
# display(labelled, line=line)
